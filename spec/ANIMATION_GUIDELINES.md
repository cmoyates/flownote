...
- When using React Native Reanimated use `transform` with `translateX` and `translateY` instead of positioning properties for hardware acceleration.

## Spring animations

- Default to spring animations when using React Native Reanimated with `withSpring()`.
